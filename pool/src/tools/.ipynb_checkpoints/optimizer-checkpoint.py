...
def get_learning_rate(args, batch_num):
    """Get learning rate"""
    return get_policy(args.sched)(args, batch_num)


def get_optimizer(args, model, batch_num):
    """Get optimizer for training"""
    print(f"=> When using train_wrapper, using optimizer {args.opt}")
    optim_type = args.opt.lower()
    ...
    # Smth code, currently I don't know why `/ 256 ` is used
    # learning_rate = learning_rate * args.batch_size * \
    #                 int(os.getenv("DEVICE_NUM", "1")) / 256.
    ...
    if optim_type == "momentum":
        ...
        #optim = torch_npu.optim.NpuFusedAdamW(model.parameters(), lr=args.lr)
...
def get_optimizer_distill(args, model,att,map, batch_num):
    """Get optimizer for training"""
    print(f"=> When using train_wrapper, using optimizer {args.opt}")
    optim_type = args.opt.lower()
    params = get_param_groups([model, att, map])
    learning_rate = get_learning_rate(args, batch_num)
    step = int(args.start_epoch * batch_num)
    train_step = len(learning_rate)
    print(f"=> Get LR from epoch: {args.start_epoch}\n"

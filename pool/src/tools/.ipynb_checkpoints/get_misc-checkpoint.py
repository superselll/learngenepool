...
from src.data.imagenet import _get_rank_info
...
def set_device(device_target, device_id):
    ...
    if device_target == "Ascend":
        if device_num > 1:
            ...
            ms.set_seed(1)
            '''
            context.set_context(device_id=int(os.environ.get("DEVICE_ID")))
...
def get_directories(
    ...
    summary_root_dir ='summary'
    ckpt_root_dir = 'checkpoints'
    best_ckpt_root_dir = 'best_checkpoints'
    last_checkpoint_root_dir = 'last_checkpoint'
    '''
    ...
    last_checkpoint_root_dir = Path(output_dir) / 'last_checkpoint'
    '''
    dir_name = f'{prefix}_{model_name}_{rank}'
    if postfix != '':
        dir_name = f'{dir_name}_{postfix}'

    summary_dir = Path(output_dir) / dir_name / summary_root_dir
    if summary_dir.exists():
        raise RuntimeError(f'Directory {summary_dir} already exist.')

    #ckpt_dir = ckpt_root_dir / dir_name
    ckpt_dir = Path(output_dir) / dir_name / ckpt_root_dir
    if ckpt_dir.exists():
        raise RuntimeError(f'Directory {ckpt_dir} already exist.')

    #best_ckpt_dir = best_ckpt_root_dir / dir_name
    best_ckpt_dir = Path(output_dir) / dir_name / best_ckpt_root_dir
    if best_ckpt_dir.exists():
        raise RuntimeError(f'Directory {best_ckpt_dir} already exist.')

    #last_checkpoint_dir = last_checkpoint_root_dir / dir_name
    last_checkpoint_dir = Path(output_dir) / dir_name/ last_checkpoint_root_dir
    ...
    last_checkpoint_dir.mkdir(parents=True)

    return str(summary_dir), str(ckpt_dir), str(best_ckpt_dir), str(last_checkpoint_dir), dir_name

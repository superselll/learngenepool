...
from src.data.augment.auto_augment import pil_interp, rand_augment_transform, augment_and_mix_transform, auto_augment_transform
...
def create_train_dataset(
    ...
    #print(f'devicenum={device_num},rankid={rank_id}')
    device_num = 1
    shuffle = True
    drop_remainder = True
...
def get_train_transforms(
    ...
    assert 0 <= min_crop < 1
    transform_img = [
        #vision.RandomCropDecodeResize(image_size,scale=(min_crop, 1.0),ratio=(3 / 4, 4 / 3),interpolation=Inter.PILCUBIC),
        vision.RandomCropDecodeResize(image_size, scale=scale, ratio=ratio,interpolation=Inter.BICUBIC)
        #vision.RandomHorizontalFlip(prob=0.5),
        #vision.ToPIL()
    ]
    if hflip>0:
        transform_img += [vision.RandomHorizontalFlip(prob=hflip)]
    if vflip>0:
        transform_img += [vision.RandomVerticalFlip(prob=vflip)]
    ...
    if re_prob>0:
        final_tfl += [RandomErasing(re_prob, mode=re_mode, max_count=re_count)]

    return transform_img+secondary_tfl+final_tfl

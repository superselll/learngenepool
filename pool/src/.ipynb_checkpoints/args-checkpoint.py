...
def _add_device_arguments(parser: argparse.ArgumentParser):
    parser.add_argument('--device_target', default='GPU', choices=['GPU', 'Ascend', 'CPU'], type=str, help='device to use for training / testing')
    parser.add_argument('--device_id', default=0, type=int, help='device id')
    parser.add_argument('--device_num', default=1, type=int, help='device num')
    parser.add_argument('--mode', default='GRAPH_MODE',choices=['GRAPH_MODE', 'PYNATIVE_MODE'])
...
def _add_dataset_arguments(parser: argparse.ArgumentParser):
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='./imagenet1k/', help='location of data.')
    parser.add_argument('--train_dir', type=str,default='train', help='Name of directory which contains training subset')
    parser.add_argument('--val_dir', type=str,default='val', help='Name of directory which contains validation subset')
    parser.add_argument('--no_dataset_sink_mode', action='store_true', help='Dataset sink mode.')
    parser.add_argument('--input_size', default=224, type=int, help='images input size')

def _add_model_arguments(parser: argparse.ArgumentParser):
    """Add arguments related to model to parser."""
    parser.add_argument('--teacher_model', default='midmap_deit_base_patch16_224', type=str,help='Name of teacher model to train ')
    parser.add_argument('--teacher_model_main', default='deit_base_patch16_224', type=str,help='Name of teacher model to main ')
    parser.add_argument('--teacher_embed_dim', type=int, default='768')
    parser.add_argument('--teacher_path', type=str, default='./teacher/deit_base_patch16_224.ckpt')
    parser.add_argument('--student_model', default='midmap_deit_tiny9_patch16_224', type=str,help='The kind of the student model in train')
    ...
    parser.add_argument('--model_ema_force_cpu', action='store_true',
                        default=False, help='')
def _add_learngene_pool_arguments(parser: argparse.ArgumentParser):
    parser.add_argument('--blk_length', type=int, default=9, help='The number of blocks in each Learngene Instance')
    parser.add_argument('--ls_init', type=bool, default=True, help='stitching layer initialization')
    parser.add_argument('--init_learngenepool_mode', default='ours', help='Initialization type of stitch layers')
    parser.add_argument('--init_stitch_mode', default='ours', help='Initialization type of stitch layers')
    parser.add_argument('--temperature', type=float, default=1.0)
    parser.add_argument('--alpha', type=float, default=0.1)
    parser.add_argument('--distill_loss', type=str, default='all', help='The kind of loss in train')
    parser.add_argument('--distillation_type', default='soft',choices=['none', 'soft', 'hard'], type=str, help='The loss type in main')
    parser.add_argument('--distillation_alpha', default=0.5, type=float, help='The distillation_alpha in main')
    parser.add_argument('--distillation_tau', default=1.0, type=float, help='The distillation_tau in main')
def _add_distilation_arguments(parser: argparse.ArgumentParser):
    """Distillation parameters"""
    parser.add_argument('--loss_pos', default='[front,end]', type=str, help='the position of supervision information between stu model and tea model')
...
def _add_learning_rate_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--decay_epochs', type=float, default=50, metavar='N',
                        help='epoch interval to decay LR')
    parser.add_argument('--warmup_epochs', type=int, default=50, metavar='N',
                        help='epochs to warmup LR, if scheduler supports')
    parser.add_argument('--cooldown_epochs', type=int, default=50, metavar='N',
                        help='epochs to cooldown LR at min_lr, '
                             'after cyclic schedule ends'
                        )
    parser.add_argument('--patience_epochs', type=int, default=50, metavar='N',
...
def _add_summary_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--keep_checkpoint_max', default=10, type=int, help='keep checkpoint max num')
    parser.add_argument('--ckpt_keep_num', type=int, default=20, help='Keep last N checkpoints.'
    )

    parser.add_argument('--ckpt_save_every_step', default=5,type=int, help='Save every N steps. To use saving by time set this to 0')
    parser.add_argument('--ckpt_save_every_sec', default=0, type=int, help='Save every N seconds. To use saving by steps set this to 0')

    parser.add_argument( '--collect_input_data', action='store_true',help='Flag to control collecting input data during training. Important: data us collected only if dataset_sink_mode is False.')
    parser.add_argument('--print_loss_every', type=int, default=20, help='Print loss every step.')
    parser.add_argument('--summary_loss_collect_freq',type=int,default=1,help='Frequency of collecting loss while training.')
...
def _add_main_train_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--bce_loss', action='store_true')  #没什么用的参数，可以删除
...
def parse_args():
    ...
    _add_learngene_pool_arguments(parser)
